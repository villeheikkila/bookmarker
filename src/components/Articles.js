...
export const Articles = ({ articles }) => {
    ...
    const sortByAuthor = () => {
        const clone = sortDirections.author ? _.sortBy(sortedArticles, ['author']) : _.sortBy(sortedArticles, ['author']).reverse()
    ...
    const sortByTitle = () => {
        const clone = sortDirections.title ? _.sortBy(sortedArticles, ['title']) : _.sortBy(sortedArticles, ['title']).reverse()
        setSortedArticles(clone)
        setSortDirections(prev => ({
            ...prev,
            title: !prev.title
    ...
    const sortByPublisher = () => {
        const clone = sortDirections.publisher ? _.sortBy(sortedArticles, ['publisher']) : _.sortBy(sortedArticles, ['publisher']).reverse()
        setSortedArticles(clone)
        setSortDirections(prev => ({
            ...prev,
    ...
    const sortByDate = () => {
        const clone = sortDirections.date ? _.sortBy(sortedArticles, function (article) {
